...
function Misc() {
  ...
  const [miscLists, setMiscList] = useState([]);
  ...
  const createLetterWordLists = () => {
    ...
    for (let i = 0; i < miscLists.length; i++) {
      internalWordLists.push(miscLists[i].id);
    }
  ...
  const queryAll = async () => {
    const colRef = collection(db, "Misc");
    const data = await getDocs(colRef);
    setMiscList(data.docs.map((doc) => ({ ...doc.data(), id: doc.id })));
  };

  return (
    <div className="flex flex-col items-center">
      <h1 className="text-4xl font-bold pt-14 pb-12 text-center">Misc</h1>
      <button
        onClick={routeChange}
