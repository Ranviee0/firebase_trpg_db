...
function Characters() {
  ...
  const [characterLists, setCharacterList] = useState([]);
  ...
  const createLetterWordLists = () => {
    ...
    for (let i = 0; i < characterLists.length; i++) {
      internalWordLists.push(characterLists[i].id);
    }
  ...
  const queryAll = async () => {
    ...
    setCharacterList(data.docs.map((doc) => ({ ...doc.data(), id: doc.id })));
  };

  return (
    <div className="flex flex-col items-center">
      <h1 className="text-4xl font-bold pt-14 pb-12 text-center">Characters</h1>
      <button
        onClick={routeChange}
